...
$(EXT_NAME).jar $(EXT_NAME).jar.pack.gz: $(SRCS) $(HTTP_CLIENT_JAR) $(HTTP_CORE_JAR) $(COMMONS_CODEC_JAR) manifest.txt Makefile
	mkdir -p classes
	$(SCALA_HOME)/bin/scalac -deprecation -unchecked -encoding us-ascii -classpath $(HTTP_CLIENT_JAR)$(COLON)$(HTTP_CORE_JAR)$(COLON)$(COMMONS_CODEC_JAR)$(COLON)$(NETLOGO)/NetLogo.jar -d classes $(SRCS)
